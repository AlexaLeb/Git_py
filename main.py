import random as r
"""
Наша программа разработана специально для выполнения группового проекта по Python. Программа является викториной, 
позволяющей создавать неограниченное количество кастомизируемых вопросов через текстовый файл, что упрощает использование 
для людей без навыков программирования. 

Принцип работы простой. Пользователь запускает программу и отвечает на вопросы по курсу Python, проверяя тем самым свои 
знания. Чем больше правильных ответов за пользователем - тем лучше результат и его оценка. Разнообразие вопросов, а 
также их вариаций по количеству правильных ответов, позволяют добавить дополнительный интерес и разнообразие в использовании 
программы. Более того, викторина является своего рода диалога между программой и пользователем, что добавляет "живой" 
язык в общении, устраняя технологические и разговорные барьеры. Также мы создали приятный и удобный интерфейс, что 
дополняет игровой опыт пользователей. 

Структура вопросов в текстовом документе: 

1. Порядковый номер вопроса
2. Тип вопроса (s - один вариант ответа, m - несколько вариантов ответа)
3. Вопрос
4. Вариант ответа №1
5. Вариант ответа №2
6. Вариант ответа №3
...
8. Правильный ответ
9. Баллы за вопрос
10. Количество попыток при ответе на вопрос

Все вопросы разделены пустой строчкой, чтобы упростить восприятие структуры текстового файла.

Очень надеемся, что пользователям понравится использование нашей программы, и они получат незабываемый игровой опыт, а 
также улучшат свои знания в прекрасном языке программирования Python! 

За каждой функцией закреплено подробное описание
"""
